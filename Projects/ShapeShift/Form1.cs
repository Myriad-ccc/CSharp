...
namespace ShapeShift
{
    public partial class Form1 : Form
    {
        ...
        private void ConfigureForm()
        {
            ...
            InitialTitleColor = Color.FromArgb(255, random.Next(256), random.Next(256), random.Next(256));
        }
        ...
        private void AddFormBorder()
        {
            ...
            FormBorder.MouseDown += (object s, MouseEventArgs e) =>
            {
                ...
                if (e.Button == MouseButtons.Right && e.X <= (int)TextRenderer.MeasureText(this.Text, new Font("Bombardier", 30f)).Width)
                {
                    if (!isFormDragging)
                    {
                        InitialTitleColor = Color.FromArgb(255, random.Next(256), random.Next(256), random.Next(256));
                        FormBorder.Invalidate();
                    }
                }
            };
        ...
        private void DrawTitle(Graphics g)
        {
            using (g)
            {
                ...
                int shadowOffset = 1;
                float fontSize = 30f;
                using (Brush shadowBrush = new SolidBrush(InitialTitleColor))
                using (Brush textBrush = new SolidBrush(Color.FromArgb(255, 235, 235, 235)))
                {
                    Font textFont = new Font("Bombardier", fontSize);
                    SizeF textSize = g.MeasureString(this.Text, textFont);

                    while (textSize.Width >= FormBorder.Width || textSize.Height >= FormBorder.Height)
                    {
                        fontSize--;
                        textFont = new Font("Bombardier", fontSize);
                        textSize = g.MeasureString(this.Text, textFont);
                    }
